...
function DashOut() {
  ...
  var postURl = `${url}/account_api/postData_out.php?name=${user}`;
  var geturl = `${url}/account_api/getDate_out.php?name=${user}`;
      ...
      .then((dates) => {
        ...
        for (let i = 0; i < dates.response.length; i++) {
          date.push({
            ...
            price: dates.response[i].total_out,
          });
        }
  ...
  const del = (nub) => {
    ...
    var delURl = `${url}/account_api/delData_out.php?id=${id}`;
    ...
    fetch(getID, {
      ...
      .then((dates) => {
        ...
        for (let i = 0; i < dates.response.length; i++) {
          date.push({
            type: dates.response[i].mtype,
            price: parseInt(dates.response[i].total_out),
          });
        }
